...
import { ScrollArea } from '@/components/ui/scroll-area'; // NECESSARY IMPORT
...
export const RequestEditor = () => {
  ...
  const copyResponse = (content: any) => {
            ...
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div className="flex-1 min-h-0">
                <ResizablePanelGroup direction="horizontal" className="h-full">
                  ...
                  {/* ======================= START OF FIXES ======================= */}
                  <ResizablePanel defaultSize={50} minSize={25} className="h-full flex flex-col">
                    <Tabs defaultValue="response" className="h-full flex flex-col">
                      ...
                      {/* Wrapper to make tab content flexible */}
                      <div className="flex-1 min-h-0">
                        {activeTab.response ? (
                            <>
                              <TabsContent value="response" asChild>
                                <ScrollArea className="h-full">
                                  ...
                                  <span className={`px-2 py-1 text-sm font-mono rounded ${
                                    ...
                                    {activeTab.response.status} {activeTab.response.statusText}
                                        ...
                                        <span className="text-sm text-gray-500">{activeTab.response.time}ms</span>
                                        <span className="text-sm text-gray-500">{(activeTab.response.size / 1024).toFixed(1)} KB</span>
                                      </div>
                                      <Button size="sm" variant="outline" onClick={() => copyResponse(JSON.stringify(activeTab.response?.data, null, 2))}>
                                        <Copy className="h-4 w-4 mr-1" />
                                        Copy
                                      </Button>
                                    </div>
                                    {/* Added classes to wrap long lines */}
                                    <pre className="bg-gray-50 p-4 rounded-md text-sm font-mono whitespace-pre-wrap break-words">
                                  ...
                                  <div className="p-4 space-y-2">
                                    {Object.entries(activeTab.response.headers).map(([key, value]) => (
                                        <div key={key} className="flex items-start gap-2 text-sm">
                                          ...
                                          {/* Added class to break long header values */}
                                          <span className="font-mono text-gray-900 flex-1 break-all">{value}</span>
                                        </div>
                                    ))}
                                  </div>
                                </ScrollArea>
