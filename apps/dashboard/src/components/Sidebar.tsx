...
export const Sidebar = () => {
  ...
  return (
      <div className={`h-full bg-white dark:bg-gray-950 border-r border-gray-200 dark:border-gray-800 flex flex-col transition-all duration-300 ${isCollapsed ? 'w-12' : 'w-80'}`}>
        ...
        {!isCollapsed && (
            <div className="flex flex-col flex-1 min-h-0">
              <div className="border-b border-gray-200 dark:border-gray-800 shrink-0 overflow-hidden">
                            ...
                            Object.entries(groupedCollections).map(([collectionName, requests]) => (
                                <div key={collectionName} className="space-y-1">
                                  <div className="flex items-center group">
                                    ...
                                    >
                                      {expandedCollections.has(collectionName) ? <FolderOpen className="h-4 w-4 mr-2 text-gray-500" /> : <Folder className="h-4 w-4 mr-2 text-gray-500" />}
                                      {editingCollection === collectionName ? (
                                          ...
                                          <>
                                            <span className="flex-1 truncate text-left">{collectionName}</span>
                                            <span className="ml-auto text-xs text-muted-foreground">{requests.length}</span>
                                          </>
                                      )}
                                    </button>
                                    {editingCollection !== collectionName && (
                                  ...
                                  {expandedCollections.has(collectionName) && (
                                      <div className="ml-4 pl-2 border-l border-gray-200 dark:border-gray-800 space-y-1">
                                        {requests.map(request => (
                                            <div key={request.id} className="flex items-center group">
                                              <button onClick={() => loadFromCollection(request)} className="flex items-center flex-1 px-2 py-2 text-sm hover:bg-muted/50 rounded-md transition-colors">
                                                <span className={`w-16 text-center px-2 py-0.5 text-xs font-mono rounded ${getMethodColor(request.method)}`}>{request.method}</span>
                      ...
                      <div className="space-y-2">
                        {history.length === 0 ? (
                            ...
                            history.map((request, index) => (
                                <button key={`history-${request.id}-${request.timestamp}-${index}`} onClick={() => loadFromCollection(request)} className="flex items-start w-full px-2 py-2 text-sm hover:bg-muted/50 rounded-md transition-colors">
                                  <span className={`w-16 text-center px-2 py-0.5 text-xs font-mono rounded shrink-0 ${getMethodColor(request.method)}`}>{request.method}</span>
                                  <div className="ml-2 flex-1 min-w-0">
                                    <div className="font-medium truncate">{request.name || 'Untitled Request'}</div>
                                    <div className="text-xs text-muted-foreground truncate">{request.url}</div>
                                    <div className="text-xs text-muted-foreground/80 mt-1">{new Date(request.timestamp).toLocaleString()}</div>
                                  </div>
                                </button>
                            ))
                        )}
                  ...
                  {activeSection === 'environments' && (
                      <div className="space-y-4">
                        ...
                        {environments.map(env => (
                            <div key={env.id} className={`space-y-2 p-3 rounded-lg border ${activeEnvironmentId === env.id ? 'border-primary/50 bg-primary/10' : ''}`}>
                              <div className="flex items-center justify-between">
                                <div className="font-medium">{env.name}</div>
                                <div className="flex items-center gap-2">
                                  {activeEnvironmentId === env.id ? <span className="text-xs bg-primary/20 text-primary px-2 py-1 rounded">Active</span> : <Button variant="outline" size="sm" onClick={() => handleUseEnvironment(env.id)} className="h-6 text-xs">Use This</Button>}
                                </div>
                              </div>
                              <div className="space-y-2">
                                {Object.entries(env.variables).map(([key, value]) => (
                                    <div key={key} className="env-row flex items-center gap-2 text-sm">
                                      {editingVariable?.envId === env.id && editingVariable?.key === key ? (
                                          <>
                                            <Input value={key} disabled className="h-7 text-xs font-mono flex-1" />
                                            <Input value={newVariableValue || String(value)} onChange={(e) => setNewVariableValue(e.target.value)} onBlur={() => handleVariableEdit(env.id, key, newVariableValue || String(value))} onKeyDown={(e) => { if (e.key === 'Enter') handleVariableEdit(env.id, key, newVariableValue || String(value)); else if (e.key === 'Escape') setEditingVariable(null); }} className="h-7 text-xs font-mono flex-1" autoFocus />
                                          </>
                                      ) : (
                                          <>
                                            <span className="text-muted-foreground flex-1 truncate">{key}:</span>
                                            <span className="flex-1 truncate">{String(value)}</span>
                                            <Button variant="ghost" size="icon" onClick={() => { setEditingVariable({ envId: env.id, key }); setNewVariableValue(String(value)); }} className="h-6 w-6 shrink-0 opacity-60 hover:opacity-100"><Edit2 className="h-3 w-3" /></Button>
                                            <Button variant="ghost" size="icon" onClick={() => handleVariableDelete(env.id, key)} className="h-6 w-6 shrink-0 opacity-60 hover:opacity-100 text-destructive"><Trash2 className="h-3 w-3" /></Button>
                                          </>
                                      )}
                                    </div>
                                ))}
